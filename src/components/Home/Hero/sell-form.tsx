...
import { BemorUzData } from "@/app/api/data"; // Adjust import as necessary
...
const SellBemorUz = () => {
  ...
  const [cryptos, setBemorUzs] = useState<BemorUz[]>([]);
  ...
  useEffect(() => {
    setBemorUzs(BemorUzData);
    if (BemorUzData.length > 0) {
      setFormData((prevData) => ({
        ...prevData,
        name: BemorUzData[0].name,
        price: BemorUzData[0].price,
      }));
    }
  }, []);
  ...
  const handleDropdownSelect = (crypto: BemorUz) => {
    setFormData((prevData) => ({
      ...prevData,
      name: crypto.name,
      price: crypto.price,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success("BemorUz purchase successful!");
